...
1. **Classify the task before checking connections.**
   ...
   - Virtual schema adapter-development workflows such as custom adapter build, source-specific JDBC dialect implementation, `virtual-schema-common-jdbc`, `SqlDialect`, metadata reader behavior, adapter properties, type mapping, pushdown capabilities, adapter JAR packaging, adapter JAR deployment, or remote debugging -> use **exasol-virtual-schema-adapter-development** behavior.
