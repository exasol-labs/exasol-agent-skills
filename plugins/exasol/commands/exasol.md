...
1. **Classify the task before checking connections.**
   - Database, general SQL outside direct `IMPORT`, export, schemas, or tables -> use **exasol-database** behavior.
   - Import workflows such as `IMPORT` or `IMPORT INTO`, local CSV or Parquet upload, remote file loading via connection objects, or `exapump upload` -> use **exasol-import** behavior.
...
3. **For any route that will run `exapump` commands:**
   ...
   - If the task is an import or upload workflow, follow **exasol-import** behavior first. Use direct `exapump sql` execution only when that guidance resolves to an executable remote-file `IMPORT` statement rather than a local-file workflow such as `exapump upload` or `IMPORT ... FROM LOCAL`.
