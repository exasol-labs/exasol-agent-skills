...
description: "Use Exasol JDBC-based virtual schemas for federated read-only access to external databases. Covers database-source adapter selection, generic JDBC fallback, CREATE VIRTUAL SCHEMA, EXPLAIN VIRTUAL, metadata refresh, and query-side troubleshooting."
---
...
Trigger when the user mentions **JDBC virtual schema**, **generic JDBC virtual schema**, **database-source federation**, **query external database through a virtual schema**, **JDBC source adapter**, **supported JDBC dialect**, or **EXPLAIN VIRTUAL** together with a JDBC/database-source virtual schema context.

## Routing Algorithm

1. **Create or query a JDBC virtual schema**
   - Trigger phrases: `JDBC virtual schema`, `generic JDBC virtual schema`, `query external database`, `database-source virtual schema`, `JDBC source adapter`
...
3. **Choose a JDBC adapter family**
   - Trigger phrases: `supported JDBC dialect`, `generic JDBC`, `dedicated adapter`, `PostgreSQL virtual schema`, `Oracle virtual schema`, `SQL Server virtual schema`, `MySQL virtual schema`
   - Load: `references/jdbc-virtual-schemas.md`

## Notes

- Use this skill for federated read-only access through JDBC/database-source virtual schemas.
