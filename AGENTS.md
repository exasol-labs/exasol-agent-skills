...
3. **check-links** — validates Markdown links in a `Docs` job shaped like Exasol `notebook-connector`'s documentation check; this Markdown-only repo runs `npx markdown-link-check` with `.github/markdown_check_config.json` instead of Notebook Connector's Poetry/Nox docs stack
