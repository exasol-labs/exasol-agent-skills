...
- `plugins/exasol/skills/*/SKILL.md` — specialized skills with routing algorithms that load only the reference files relevant to the task (progressive disclosure). Skills: `exasol` (top-level router), `setup-personal` (guided AWS deployment), `exasol-database` (SQL/exapump), `exasol-import-export` (native IMPORT/EXPORT and exapump file movement), `exasol-udfs` (UDFs/SLCs), `exasol-bucketfs` (BucketFS), `exasol-ai-setup` (notebook-connector setup), `exasol-itde` (local Docker Exasol lifecycle), `exasol-notebook-connections` (Python connection helpers), `exasol-transformers` (Transformers Extension)
